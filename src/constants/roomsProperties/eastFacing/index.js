    ...
    setTimeout(() => {
      ...
      store.dispatch(
        updateRoomData({
          ...
          position: { top: Math.floor(extraBathBreadth) * scale - setbacks.back * scale, right: Math.floor(bed1Length) * scale }
        })
      )
    }, 100)
    ...
    setTimeout(() => {
      store.dispatch(
        updateRoomData({
          id: '',
          roomType: 'dining',
          position: { left: Math.floor(bed3Length) * scale + 18, bottom: Math.floor(staircaseLength + drawingBreadth / 5) * scale }
        })
      )
...
function generateTypeB() {
  ...
  if (plotBreadth >= 25 && plotBreadth <= 35) {
    if (plotLength >= 60 && plotLength <= 65) {
      ...
      setTimeout(() => {
        ...
        // position: { bottom: Math.floor(poojaBreadth) *scale-6, left:0 }}))
        store.dispatch(
          updateRoomData({ id: 'sw', roomType: 'toilet', length: toilet4Length, breadth: toilet4Breadth, position: { top: Math.floor(bed4Breadth) * scale - setbacks.front * (scale - 4), bottom: 0 } })
        )
  ...
  if (plotLength >= 60 && plotLength <= 70) {
    //Breadth :60-70
    const bed1Length = builtLength * 0.2
    ...
    setTimeout(() => {
      ...
      store.dispatch(
        updateRoomData({ id: 'nw', roomType: 'toilet', length: toilet1Length, breadth: toilet1Breadth, position: { left: -40, top: Math.floor(stairCaseBreadth + diningBreadth) * scale } })
      )
      store.dispatch(updateRoomData({ id: 'ne', roomType: 'toilet', length: toilet2Length, breadth: toilet2Breadth }))
      // position: { left: Math.floor(bed1Length-setbacks.left) *scale, top:0 }}))
      store.dispatch(updateRoomData({ id: 'se', roomType: 'toilet', length: toilet3Length, breadth: toilet3Breadth, position: { bottom: 0, left: 0 } }))
